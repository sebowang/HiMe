...
Supported providers and default (mid-tier) models — April 2026
---------------------------------------------------------------
- ``gemini``         → gemini-3-flash-preview              (alt: gemini-3.1-flash-lite-preview / gemini-3.1-pro-preview)
- ``google_vertex``  → gemini-3-flash-preview              (same lineup, hosted on Vertex AI)
- ``openai``         → gpt-5.4-mini                        (alt: gpt-5.4-nano / gpt-5.4)
- ``azure_openai``   → gpt-5-mini                          (Azure deployment name; deploy first)
...
- ``deepseek``       → deepseek-flash                      (alt: deepseek-v4-pro; both 1M ctx, dual-mode thinking)
...
- ``amazon_bedrock`` → amazon.nova-pro-v1:0                (alt: nova-micro / nova-lite, or anthropic.claude-* on Bedrock)
- ``minimax``        → MiniMax-M2                          (alt: abab6.5s-chat / MiniMax-M2.5)
- ``zhipuai``        → glm-4.7                             (alt: glm-4-air / glm-4.7-flash / glm-5)
- ``vllm``           → resolved from VLLM_MODEL env var (any model served by a local vLLM server)
"""
...
def create_provider(
    ...
    **kwargs:
    ...
    # Defaults favour the lowest-effort reasoning to keep agentic loops fast:
    #   - Anthropic: 1024 is the API-enforced minimum for extended thinking.
    #   - Gemini: 0 disables thinking on 2.5-flash; the provider clamps to 128
    #     on 2.5-pro (which cannot fully disable) and uses thinking_level on 3.x.
